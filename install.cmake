...
set(DEPS_FILE ${CMAKE_SOURCE_DIR}/cpack-deps/${DISTRO_NAME}-${DISTRO_VER})

if (EXISTS ${DEPS_FILE})
    execute_process(COMMAND cat ${DEPS_FILE} OUTPUT_VARIABLE PACKAGE_DEPS OUTPUT_STRIP_TRAILING_WHITESPACE)
    message(STATUS "Dependencies which will be used during packaging monolithic: ${PACKAGE_DEPS}")
else (EXISTS ${DEPS_FILE})
    message(WARNING "Dependencies during packaging monolithic not be used because of file '${DEPS_FILE}' don't exists!")
...
foreach(component ${CPACK_COMPONENTS_ALL})
	set(COMPONENT_DEPS_FILE ${CMAKE_SOURCE_DIR}/cpack-deps/${DISTRO_NAME}-${DISTRO_VER}-${component})

	set(COMPONENT_PACKAGE_DEPS)
        if(EXISTS ${COMPONENT_DEPS_FILE})
	    execute_process(COMMAND cat ${COMPONENT_DEPS_FILE} OUTPUT_VARIABLE COMPONENT_PACKAGE_DEPS OUTPUT_STRIP_TRAILING_WHITESPACE)
	    message(STATUS "Dependencies which will be used during packaging component ${component}: ${COMPONENT_PACKAGE_DEPS}")
        else()
            STRING(REGEX MATCH "(.+)-dbg" TMP_COMPONENT_NAME ${component})
            if(TMP_COMPONENT_NAME)
                set(TMP_COMPONENT_NAME ${CMAKE_MATCH_1})
                STRING(CONFIGURE ${CPACK_CONFIGURE_VERSION_STRING} COMPONENT_PACKAGE_DEPS)
                message(STATUS "Dependencies which will be used during packaging component ${component}: ${COMPONENT_PACKAGE_DEPS}")
            else()
                message(WARNING "Dependencies during component ${component} packaging will not be used because of file '${COMPONENT_DEPS_FILE}' don't exists!")
            endif()
        endif()
