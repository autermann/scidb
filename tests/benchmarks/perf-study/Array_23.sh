...
echo "||                                           ||"
echo "||                Array_23.sh                ||"
echo "|| SPARSE(1%) Array, Zipf (0.1) Distribution ||"
echo "||                                           ||"
echo "+=============================================="
echo ""
date
...
gen_matrix -r $1 $1 $2 $2 0.01 0.1 DDF > /tmp/Load.pipe &
#
/usr/bin/time -f "Array_23 %e" iquery -naq "load(Test_Array, '/tmp/Load.pipe')"
#
#  DML 1: How many cells? 
#
/usr/bin/time -f "Array_23 Cell_Count %e" iquery -taq "
join ( 
        build ( < s : string > [ I=0:0,1,0 ], 'Array_23'),
        count ( Test_Array )
)"
#
#  Size
