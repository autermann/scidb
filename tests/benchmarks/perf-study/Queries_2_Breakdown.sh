#!/bin/sh
#
#  File: Queries_2_Breakdown.sh 
#
#   This script is intended to run against the Array_2 arrays, which has the 
...
#     double_attr : double
#  >
#  [ I=0:39999,10000,0, J=0:39999,10000,0 ]"
#
...
join (
  aggregate ( between(Test_Array, 600, 600, 20600, 20600), 
              count(*)),
  join ( 
  	aggregate ( between(Test_Array, 7600, 7600, 27600, 27600), 
                    count(*)),
        aggregate ( between(Test_Array, 19600, 19600, 39600, 39600), 
                    count(*))
  )
)"
#
