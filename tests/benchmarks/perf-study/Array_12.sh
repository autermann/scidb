...
echo "||                                        ||"
echo "||              Array_12.sh               ||"
echo "|| DENSE Array, Zipf (0.5) Distribution   ||"
echo "||                                        ||"
echo "+==========================================="
echo ""
date
...
#  This array is divided into 16 chunks, each of which has 1,000,000 elements. 
...
gen_matrix -r $1 $1 $2 $2 1.0 0.5 DDF > /tmp/Load.pipe &
#
/usr/bin/time -f "Array_12 Load %e" iquery -naq "
load(Test_Array, '/tmp/Load.pipe')"
#
/usr/bin/time -f "Array_12 Cell_Count %e" iquery -taq "
join ( 
        build ( < s : string > [ I=0:0,1,0 ], 'Array_12'),
        count ( Test_Array )
)"
#
#  Size
