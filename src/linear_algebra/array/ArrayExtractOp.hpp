                     ...
                     ExtractOp_tt& extractOp, const shared_ptr<Query>& query)
{
    ...
    for(chunksIt = array->getConstIterator(/*attrid*/0); ! chunksIt->end(); ++(*chunksIt) ) {
        ...
        const size_t INTEL_L1_DATA_CACHE_BYTES = 32*1024 ;  // This hasn't changed for Intel for years and years
                                                            // we don't have access from Linux to the L1 size,
                                                            // (only the last-level-cache size, not what we want here)
        const size_t MAX_VALUES_TO_GET = INTEL_L1_DATA_CACHE_BYTES/2/sizeof(Value_t);  // use about 1/2 of L1, the other half is for the destination
