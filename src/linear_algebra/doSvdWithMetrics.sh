...
echo "@@@@ $0: @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
echo "@@@@ $0: matrix orders: $ORDER_LIST"
  ...
  for CSIZE in $CHUNKSIZES ; do
    ...
    if [ ${TEST} == "mpicopy" -o ${TEST} == "mpirank" ] ; then
        ...
        if [ "$ORDER" -le "128" ] ; then # really for debugging, run enough just to keep this case working
            ...
            if [ "$TEST" == "mpicopy" ] ; then
                echo "@@@@ $0: -----------------------------------------------"     #|tee /dev/stderr
                echo "@@@@ $0: mpi(scalapack)copy , size ${ORDER}, csize ${CSIZE}"  #|tee /dev/stderr
            ...
            if [ "$TEST" == "mpirank" ] ; then
                ...
                echo "@@@@ $0: -----------------------------------------------"     #|tee /dev/stderr
                echo "@@@@ $0: mpi(scalapack)rank , size ${ORDER}, csize ${CSIZE}"  #|tee /dev/stderr
    ...
    if [ ${TEST} == "svd" ] ; then
      if [ "$ORDER" -gt "2048" ] ; then
        echo "@@@@ $0: SVD(A) size ${ORDER}, chunksize ${CSIZE}"   |tee /dev/stderr
        # don't bother saving and running SvdMetric,
        # plain svd is too slow to compare to anyway

        echo "$0: building test matrix"
        nafl -q "store(${BUILD},INPUT)"

        echo "$0: compute 3 svd matrices: values, left, right, no metrics (too large):" 
        ...
        echo "$0: svd with check, size ${ORDER}, csize ${CSIZE}"    |tee /dev/stderr
        echo "$0: building test matrix"
        nafl -q "store(${BUILD},INPUT)"
        ...
        SUM_SQUARES='sum(project(apply(DOSVD_METRIC,square,nelsb*nelsb),square))'
