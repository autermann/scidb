...
#include <proto/MessageTypes.h>
#include <proto/scidb_msg.pb.h>
#include <util/shm/SharedMemoryIpc.h>

// scidb internal
#include <../src/network/BaseConnection.h>
                      ...
                      uint64_t launchId)
{
    ...
    while (scidbCommand.getCmd() != scidb::mpi::Command::EXIT) {
        ...
        if(scidbCommand.getCmd() == "DLAOP") {
            ...
            // this is where we do something, right?
            if(dlaOp == "pdgesvd_") {
...
void handleAbnormalExit(const std::vector<std::string>& args)
{
    ...
    if (args.size() != 1) {
        cerr << "SLAVE: NUMARGS for ABNORMAL_EXIT is invalid" << std::endl;
        _exit(99);
    }
