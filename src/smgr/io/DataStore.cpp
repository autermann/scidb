                     ...
                     size_t allocatedSize)
{
    ...
    {
        _dirty = true;
        DataStoreFlusher::getInstance()->add(_guid);
    }
}
...
DataStore::freeChunk(off_t off, size_t allocated)
{
     ...
     */
    _freelists[allocated].insert(off);
}
...
DataStore::persistFreelists()
{
    ...
    LOG4CXX_TRACE(logger, "datastore: persisting freelist for " << _file->getPath() << " buckets " << nbuckets);   
 ...
 */
DataStore::DataStore(char const* filename, Guid guid) :
    _dslock(),
    _guid(guid),
    _largestFreeChunk(0),
    _dirty(false),
    _fldirty(false)
{
    /* Initialize the manager reference
     */
    _dsm = DataStores::getInstance();
    ...
    if (_file->fstat(&st) != 0)
    {
        throw SYSTEM_EXCEPTION(SCIDB_SE_STORAGE,
            ...
            << "fstat" << -1 << errno << filenamestr; 
    }    
    st.st_size = roundUpPowerOf2(st.st_size);
    if (st.st_size > (off_t)_allocatedSize)
    {
        _allocatedSize = st.st_size;
    }

    /* Try to initialize the free lists from the free-list file.
       If we fail, assume that if the file is non-empty, then it is fully
       allocated.
     */
...
DataStores::initDataStores(char const* basepath)
{
    ...
    if (_theDataStores == NULL)
    {
        ...
        if (syncMSeconds > 0)
        {
            DataStoreFlusher::getInstance()->start(syncMSeconds);
        }
...
DataStores::getDataStore(DataStore::Guid guid)
{
    ...
    retval = boost::make_shared<DataStore>(filepath.str().c_str(), guid);
 ...
 */
void
DataStoreFlusher::DataStoreFlushJob::run()
{
    while (true)
    {
        int64_t totalSyncTime= 0;
        {
            ...
            for (it = dss.begin(); it != dss.end(); ++it)
            {
                ...
                ds = DataStores::getInstance()->getDataStore(*it);
                ds->flush();
...
DataStoreFlusher::start(int timeIntervalMSecs)
{
    ...
    _myJob.reset(new DataStoreFlushJob(timeIntervalMSecs, this));
