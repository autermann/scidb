...
{
public:
	LogicalStdev(const std::string& logicalName, const std::string& alias):
    ...
    ArrayDesc inferSchema(std::vector< ArrayDesc> schemas, boost::shared_ptr< Query> query)
    {
        ...
        AggregatePtr stdevAggregate = AggregateLibrary::getInstance()->createAggregate("stdev", TypeLibrary::getType(attrs[aid].getType()));
        Attributes aggAttrs(1);
        aggAttrs[0] = AttributeDesc((AttributeID)0,
                                    attrs[aid].getName() + "_stdev",
                                    stdevAggregate->getResultType().typeId(),
...
DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalStdev, "stdev")
