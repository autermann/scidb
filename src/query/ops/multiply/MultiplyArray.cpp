...
namespace scidb
{
    ...
    //
    // MultiplyArrayIterator methods
    //
    void MultiplyArrayIterator::operator ++()
    {
        if (!hasCurrent)
    ...
    bool MultiplyArrayIterator::end()
    {
        return !hasCurrent;
    }

    Coordinates const& MultiplyArrayIterator::getPosition()
    {
        return currPos;
    }

    bool MultiplyArrayIterator::setPosition(Coordinates const& pos)
    {
        Dimensions const& dims = array.dims;
    ...
    void MultiplyArrayIterator::reset()
    {
        Dimensions const& dims = array.dims;
        hasCurrent = true;
        for (size_t i = 0, n = currPos.size(); i < n; i++) {
    ...
    /***
     * This is the main method in MultiplyArrayIterator that computes a whole output chunk
     * of the multiplication and sends it up the pipeline
     */
    ConstChunk const& MultiplyArrayIterator::getChunk()
    {
        if (!hasCurrent)
            throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_CURRENT_ELEMENT);
    ...
    ArrayDesc const& MultiplyArray::getArrayDesc() const
    {
        return desc;
    }
    ...
    boost::shared_ptr<ConstArrayIterator> MultiplyArray::getConstIterator(AttributeID attr) const
    {
        return result ? result->getConstIterator(attr)
            : boost::shared_ptr<ConstArrayIterator>(isSparse
                                             ? (ConstArrayIterator*)new MultiplySparseArrayIterator(*this, attr)
                                             : (ConstArrayIterator*)new MultiplyArrayIterator(*this, attr));
...
//        if (leftChunk.isPlain() && rightChunk.isPlain()) 
    ...
    void MultiplyArray::multiplyChunks(void const* leftData,
    ...
    MultiplyArray::MultiplyJob::MultiplyJob
        (MultiplyArray const& arr,
         void const* left,
         void const* right,
         void* result,
    ...
    void MultiplyArray::MultiplyJob::run()
    {
    ...
    //
    // MultiplySparseArrayIterator
    //
    ConstChunk const& MultiplySparseArrayIterator::getChunk()
    {
        if (!hasCurrent)
            throw USER_EXCEPTION(SCIDB_SE_EXECUTION, SCIDB_LE_NO_CURRENT_ELEMENT);
        // Initialize the chunk
    ...
    MultiplySparseArrayIterator::MultiplySparseArrayIterator(MultiplyArray const& array, AttributeID id)
    : MultiplyArrayIterator(array, id),
