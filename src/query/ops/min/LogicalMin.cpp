...
/*
 * LogicalMin.cpp
 * Example query : min (scan ('array_to_scan'), 'attribute_to_aggregate' )
...
{
public:
	LogicalMin(const string& logicalName, const std::string& alias):
    ...
    ArrayDesc inferSchema(std::vector< ArrayDesc> schemas, boost::shared_ptr< Query> query)
    {
        ...
        AggregatePtr minAggregate = AggregateLibrary::getInstance()->createAggregate("min", TypeLibrary::getType(attrs[aid].getType()));
		Attributes aggAttrs(1);
		aggAttrs[0] = AttributeDesc((AttributeID)0,
		                            attrs[aid].getName() + "_min",
                                    minAggregate->getResultType().typeId(),
...
DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalMin, "min")
