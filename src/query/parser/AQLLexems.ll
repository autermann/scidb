...
#define YY_DECL                             \
    short scidb::AQLScanner::lex(           \
        AQLParser::semantic_type* yylval,   \
        AQLParser::location_type* yylloc)
%}
...
{Identifier} {
    if (const ALKeyword* kw = getAQLKeyword(yytext))
    {
...
scidb::AQLScanner::AQLScanner(QueryParser& glue,std::istream* in,std::ostream* out)
    : AQLBaseFlexLexer(in,out),
      _glue(glue),
      _arena("AQLScanner")
{}

void scidb::AQLScanner::set_debug(bool b)
{
    yy_flex_debug = b;
}

char* scidb::AQLScanner::copyLexeme(size_t n,const char* p)
{
