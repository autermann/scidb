 ...
 *
 * @par Synopsis: index_lookup (input_array, index_array, input_array.attribute_name
 *                              [,output_attribute_name] [,'memory_limit=MEMORY_LIMIT'])
 *
 * @par Examples:
...
public:
    ...
    ArrayDesc inferSchema(vector< ArrayDesc> schemas, shared_ptr< Query> query)
    {
        ...
        //The settings object also checks the schemas for validity this time
        IndexLookupSettings settings(input, index, _parameters, true, query);

        //Get input attributes without the empty tag
        Attributes outputAttributes = input.getAttributes(true);

        //Add new attribute
        outputAttributes.push_back(AttributeDesc(outputAttributes.size(),
                                                 settings.getOutputAttributeName(),
                                                 TID_INT64,
                                                 AttributeDesc::IS_NULLABLE,
                                                 0));

        //Then add empty tag if input had it
        //Empty tag must always be the last attribute
        if (input.getEmptyBitmapAttribute())
        {
            outputAttributes = addEmptyTagAttribute(outputAttributes);
        }
        return ArrayDesc(input.getName(), outputAttributes, input.getDimensions());
    }
};

REGISTER_LOGICAL_OPERATOR_FACTORY(LogicalIndexLookup, "index_lookup");
