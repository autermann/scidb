...
static log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("scidb.toy_operators.uniq"));
...
public:
    ...
    /**
     * A map from Coordinate to InputChunkInfo objects with added marshalling routines
     * XXX: If exporting this to a header for general use, do NOT inherit from stl map, use containment.
     */
    class InputArrayInfo : public map<Coordinate, InputChunkInfo>
    {
    private:
        size_t _marshalledSize; //maintained as data is added

    public:
        typedef map<Coordinate, InputChunkInfo> Super;
        ...
        /**
         * Add a chunkInfo to this and update the internally maintained size.
         * TODO: override / make private other insertion methods so they are not called by accident.
         * @param chunkCoord the coordinate of the chunk
         * @param chunkInfo the data for the chunk
         */
        void addInfo(Coordinate const chunkCoord, InputChunkInfo const& chunkInfo)
        {
            _marshalledSize += (chunkInfo.getMarshalledSize() + sizeof(Coordinate));
            insert( pair<Coordinate,InputChunkInfo>(chunkCoord, chunkInfo) );
        }
         ...
         */
        shared_ptr<SharedBuffer> marshall() const
        {
            shared_ptr <SharedBuffer> result (new MemoryBuffer(NULL, _marshalledSize));
         ...
         */
        void unMarshall(shared_ptr<SharedBuffer>& buf)
        {
            ...
            for ( ; numEntries > 0; --numEntries)
            {
                ...
                ++cptr;
                InputChunkInfo chunkInfo;
                sptr = reinterpret_cast<size_t*>(chunkInfo.unmarshall(cptr));
                EXCEPTION_ASSERT(find(coord)==end());
                addInfo(coord, chunkInfo);
            }
        }
        ...
        {
            LOG4CXX_DEBUG(logger, "InputArrayInfo dump:");
            ...
            for(const_iterator iter = begin(); iter!= end(); ++iter)
            {
                ...
                {
                    LOG4CXX_DEBUG(logger, ">>{"<<coord<<"} -> values 0");
                }
                else if (converter != NULL)
                {
                    ...
                    LOG4CXX_DEBUG(logger, ">>{"<<coord<<"} -> values "<<chunkInfo.numUniqueElements
     ...
     * TODO: consider using a template to unify the two classes.
     * XXX: If exporting this to a header for general use, do NOT inherit from stl map, use containment.
     */
    class OutputArrayInfo : public map<Coordinate, OutputChunkInfo>
    {
    private:
        size_t _marshalledSize;
    ...
    public:
        ...
        void addInfo(Coordinate const& chunkCoord, OutputChunkInfo const& outputChunkInfo)
        {
            _marshalledSize += (sizeof(Coordinate) + sizeof(OutputChunkInfo));
            insert(pair<Coordinate, OutputChunkInfo>(chunkCoord, outputChunkInfo));
        }
        ...
        shared_ptr<SharedBuffer> marshall() const
        {
            ...
            *sptr = size();
            ...
            for(const_iterator iter = begin(); iter!= end(); ++iter)
            {
                ...
                *cptr = iter->first;
                ++cptr;
                *cptr = iter->second.startingPosition;
                ++cptr;
                bool* bptr = reinterpret_cast<bool*>(cptr);
                *bptr = iter->second.writeFirstValue;
                ++bptr;
                sptr = reinterpret_cast<size_t*>(bptr);
            }
            return result;
        ...
        void unMarshall(shared_ptr<SharedBuffer>& buf)
        {
            ...
            {
                Coordinate* cptr = reinterpret_cast<Coordinate*> (sptr);
                Coordinate inputChunkCoord = *cptr;
                ++cptr;
                OutputChunkInfo chunkInfo;
                chunkInfo.startingPosition = *cptr;
                ++cptr;
                bool* bptr = reinterpret_cast<bool*>(cptr);
                chunkInfo.writeFirstValue = *bptr;
                ++bptr;
                addInfo(inputChunkCoord, chunkInfo);
                sptr = reinterpret_cast<size_t*> (bptr);
            }
        }
        ...
        {
            LOG4CXX_DEBUG(logger, "OutputArrayInfo dump:");
            for(const_iterator iter = begin(); iter!= end(); ++iter)
            {
                Coordinate coord = iter->first;
                OutputChunkInfo const& chunkInfo = iter->second;
                LOG4CXX_DEBUG(logger, ">>{"<<coord<<"} -> outputCoord {"<<chunkInfo.startingPosition<<"} writeFirst "
     ...
     */
    class OutputArraySequentialWriter
    {
    private:
    ...
    void fillInputArrayInfo(shared_ptr<Array>& inputArray, InputArrayInfo& inputArrayInfo)
    {
        ...
        while(!inputArrayIter->end())
        {
            ...
            inputArrayInfo.addInfo(chunkCoord, inputChunkInfo);
            ++(*inputArrayIter);
        }
    }
    ...
    void computeOutputArrayInfo(InputArrayInfo const& inputArrayInfo, OutputArrayInfo& outputArrayInfo)
    {
        ...
        for (InputArrayInfo::const_iterator iter = inputArrayInfo.begin();
             iter != inputArrayInfo.end();
             ++ iter)
        {
            ...
            outputArrayInfo.addInfo(inputChunkPosition, outputInfo);
        }
    }

    /**
     * Given an inputArrayInfo from every instance, marshall it, send it to the coordinator, on coordinator merge
     * and compute the outputArrayInfo; then broadcast to all instances.
    ...
    void exchangeArrayInfo(InputArrayInfo& inputArrayInfo, OutputArrayInfo& outputArrayInfo, shared_ptr<Query>& query)
    {
        if (query->getInstanceID() != 0)
        {
            ...
            BufSend(0, buf, query);   //send to instance 0
            buf = BufReceive(0,query); //receive from instance 0
            outputArrayInfo.unMarshall(buf);
        }
        else
        {
            ...
            computeOutputArrayInfo(inputArrayInfo, outputArrayInfo); //compute output; send to everyone
            shared_ptr<SharedBuffer> buf = outputArrayInfo.marshall();
            for (InstanceID i = 1; i<query->getInstancesCount(); ++i)
            {
                BufSend(i, buf, query);
            }
                                       ...
                                       shared_ptr<Query>& query)
    {
             ...
             ++(*inputArrayIter))  //for each chunk in input
        {
            ...
            OutputChunkInfo const& outputChunkInfo = iter->second;
            if (outputChunkInfo.startingPosition < 0)
            {   //we are told to skip the chunk
                continue;
            }
            Coordinate currentOutputPos = outputChunkInfo.startingPosition; //write data to output starting at this pos
                ...
                ++(*inputChunkIter)) //for each value in the chunk
            {
                ...
                {
                    if(outputChunkInfo.writeFirstValue || !lastVal.isNull())
                    {
...
REGISTER_PHYSICAL_OPERATOR_FACTORY(PhysicalUniq, "uniq", "PhysicalUniq");
